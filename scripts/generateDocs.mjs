...
/**
 * Replace all instances of @aurodesignsystem, icon, Icon, auro and Auro accordingly
 */
...
function formatTemplateFileContents(content, destination) {
  ...
  result = result.replace(/\icon(?!\()/g, nameExtractionData.name);
  result = result.replace(/\Icon(?!\()/g, nameExtractionData.nameCap);
